...
impl ProofDB {
    ...
    pub fn new(db: Rc<Rexie>, unit: CurrencyUnit) -> Self {
    ...
    async fn store_proof(&self, proof: ProofEntry) -> Result<PublicKey> {
        ...
        let key = JsValue::from_str(&proof.y.to_string());
        proofs.add(&entry, Some(&key)).await?;
        tx.done().await?;
        Ok(proof.y)
    }

    #[allow(dead_code)]
    async fn delete_proof(&self, index: PublicKey) -> Result<()> {
    ...
    async fn counter(&self, kid: cdk02::Id) -> Result<CounterEntry> {
        ...
        let entry = if let Some(entry) = response {
            ...
            let entry = counters_repo.add(&entry, Some(&key)).await?;
            from_value::<CounterEntry>(entry)?
        };
        tx.done().await?;
        Ok(entry)
    }
    async fn increment_counter(&self, old: CounterEntry, new: CounterEntry) -> Result<()> {
        if old.kid != new.kid {
            return Err(Error::Any(AnyError::msg(
                "rexie::increment_counter input kid mismatch",
            )));
        }
        let key = JsValue::from_str(&old.kid.to_string());
        let tx = self
            .db
            .transaction(&[self.counter_store.clone()], TransactionMode::ReadWrite)?;
        let counters_repo = tx.store(&self.counter_store)?;
        ...
        let entry = from_value::<CounterEntry>(response.unwrap())?;
        ...
        counters_repo.put(&new_entry, Some(&key)).await?;
        tx.done().await?;
        Ok(())
    }
}
...
impl PocketRepository for ProofDB {
    ...
    async fn increment_counter(&self, kid: cdk02::Id, old: u32, increment: u32) -> Result<()> {
        ...
        self.increment_counter(old, new).await?;
        Ok(())
    }
}
