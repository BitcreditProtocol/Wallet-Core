...
use std::{cell::RefCell, collections::HashSet, rc::Rc, str::FromStr, sync::Arc};
...
pub struct AppState {
    wallets: Vec<Arc<ProductionWallet>>,
...
pub async fn add_wallet(
    ...
    let index = APP_STATE.with_borrow_mut(|state| {
        state.wallets.push(Arc::new(new_wallet));
        state.wallets.len() - 1
    });
    Ok(index)
}
...
pub fn wallet_name(idx: usize) -> Result<String> {
    ...
    let wallet: Arc<ProductionWallet> =
        APP_STATE.with_borrow(|state| -> Result<Arc<ProductionWallet>> {
...
pub fn wallets_ids() -> Result<Vec<u64>> {
    ...
    let ids = APP_STATE.with_borrow(|state| {
        state.wallets.iter().enumerate().map(|(i, _)| i as u64).collect::<Vec<_>>()
    });
    Ok(ids)
}
...
pub fn wallets_names() -> Result<Vec<String>> {
    ...
    let names = APP_STATE.with_borrow(|state| {
        state.wallets.iter().map(|w| w.name.clone()).collect::<Vec<_>>()
    });
    Ok(names)
}
