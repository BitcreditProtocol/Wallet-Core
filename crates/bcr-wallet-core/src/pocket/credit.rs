...
impl Pocket {
    ...
    async fn digest_intermint_proofs(
        ...
        if response.ids.len() != kids.len() {
            return Err(Error::Any(AnyError::msg(
                "intermint swap response ids length mismatch",
            )));
        }
        let kids_map: HashMap<cashu::Id, cashu::Id> =
            HashMap::from_iter(kids.iter().cloned().zip(response.ids.iter().cloned()));
